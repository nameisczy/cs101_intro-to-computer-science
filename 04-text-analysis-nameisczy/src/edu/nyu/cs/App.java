...
public class App {
  ...
  public static void main(String[] args) throws Exception {
    ...
    String text = getContentsOfFile(getFilepathFromUser());
    String[] tics = getTicsFromUser();
    int total=0;
    System.out.println("\n"+"...............................Analyzing text................................."+"\n");
    for(int i=0;i< tics.length;i++){
      total+=countOccurrences(tics[i],text);
    }
    System.out.println("Total number of tics: "+total);
    double densityBefore = calculateTicDensity(tics,text);
    String density = String.format("%.2f", densityBefore);
    System.out.println("Density of tics: "+ density);
    System.out.println("\n"+"...............................Tic breakdown.................................."+"\n");
    for(int i=0;i<tics.length;i++){
      int Occurrence = countOccurrences(tics[i],text);
      System.out.println(String.format("%-10s", tics[i])+"/ "+String.format("%-20s",countOccurrences(tics[i],text)+" occurrences")+"/ "+calculatePercentage(Occurrence,total)+"% of all tics");
    }
 ...
 /**
   ...
   */
    ...
    public static int countOccurrences(String needle, String haystack){
      ...
      String[] haystackAfter = haystack.split("\\W+");
      for (int i=0; i < haystackAfter.length; i++){
        if (haystackAfter[i].equals(needle)){
          count ++;
        }
      }
      return count;
   ...
   */
    ...
    public static int calculatePercentage(int num1, int num2){
      double percentageBefore = (double)num1/num2;
      double percentage = percentageBefore * 100;
    ...
    public static double calculateTicDensity(String[] tics, String fullText){
      ...
      for (int i=0; i < tics.length; i++){
        for (int ii=0; ii < total; ii++){
          if (tics[i].toLowerCase().equals(words[ii])){
            count++;
          }
        }
      }
      double ticDensity = (double) count/total;
      return ticDensity;
    }
